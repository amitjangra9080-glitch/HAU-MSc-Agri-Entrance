...
function cleanEnvText(value) {
  ...
  // Accept values accidentally pasted with JSON quotes and/or a trailing comma.
...
function normalizePrivateKey(value) {
  ...
  // Remove accidental JSON punctuation or copied text outside the PEM block.
...
function normalizeServiceAccount(value) {
  ...
  if (!/^[a-z][a-z0-9-]{4,29}$/.test(projectId)) {
    throw new ServerCredentialError("invalid_project_id", "Firebase project ID is invalid.");
  }
  if (!/^[^\s@]+@[^\s@]+\.iam\.gserviceaccount\.com$/.test(clientEmail)) {
    throw new ServerCredentialError("invalid_client_email", "Firebase client email is invalid.");
  }
...
export function parseServiceAccountEnv(environment = process.env) {
  ...
  if (jsonCredential) {
    try {
      ...
      // A stale or malformed JSON variable must not block correctly configured split variables.
...
export function credentialFailureReason(error) {
  ...
  if (message.includes("private key") || message.includes("pem") || message.includes("asn.1")) {
